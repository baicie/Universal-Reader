...
- ✅ 新增格式边界测试（大小写排序、嵌套目录、非图片过滤、空压缩包）
- ✅ 测试覆盖率：`comic_document.dart` 从 30/52 行提升至 52/52 行

**覆盖的 API：**
- 页面导航（`pageIndex`、`chapterCount`、`currentPage`）
...
**测试结果：**
- 所有 483 个测试通过（从 460 增加到 483）
- `comic_document.dart` 覆盖率：**100%（52/52 行）**
