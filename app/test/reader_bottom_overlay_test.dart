...
void main() {
  ...
  testWidgets('forwards save and dismiss callbacks', (tester) async {
    ...
    await tester.pumpAndSettle();
    final bar = tester.widget<SelectionConfirmBar>(find.byType(SelectionConfirmBar));
    bar.onSave();
    bar.onDismiss();
    expect(saved, 1);
    expect(dismissed, 1);
  ...
  testWidgets('forwards progress label and seek callbacks', (tester) async {
    ...
    final bar = tester.widget<ReaderProgressBar>(find.byType(ReaderProgressBar));
    bar.onSeek(0.7);
...
const _annotatedQuoteKey = annotatedQuoteKey; // keep import alive if structure shifts
