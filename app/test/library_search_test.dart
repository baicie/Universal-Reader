...
void main() {
  group('librarySearchAll', () {
    test('empty query returns all documents as metadata hits', () {
      ...
      expect(
        hits.every((hit) => hit.kind == LibrarySearchHitKind.metadata),
        isTrue,
      );
      expect(
        hits.map((hit) => hit.document.metadata.id).toSet(),
        equals({'a', 'b'}),
      );
    });
    ...
    test('does not match by internal id', () {
      // Internal ids are not user-facing copy; searching by them leaks an
      // unrelated field that the user can never see in the UI.
      final docs = [_doc(id: 'abc123', title: 'Alpha')];

      expect(librarySearchAll(documents: docs, query: 'abc123'), isEmpty);
    });

    test('matches by title case-insensitively', () {
      final docs = [
        _doc(id: 'design', title: 'Design Patterns'),
        _doc(id: 'plain', title: 'Notes'),
      ];

      final hits = librarySearchAll(documents: docs, query: 'design').toList();
    ...
    test('matches by author as well as title', () {
      ...
      final hits = librarySearchAll(documents: docs, query: 'Haruki').toList();
    ...
    test('returns each document at most once even if both fields match', () {
      final docs = [_doc(id: 'book', title: 'match', author: 'match')];

      final hits = librarySearchAll(documents: docs, query: 'match').toList();
      expect(hits, hasLength(1));
      expect(hits.first.kind, LibrarySearchHitKind.metadata);
    });
  });
  ...
  group('librarySearchAllAsync', () {
    ...
    test(
      'does not duplicate a document between metadata and note hit',
      () async {
        final docs = [_doc(id: 'a', title: 'Alpha match')];

        final hits = await librarySearchAllAsync(
          documents: docs,
          query: 'match',
          annotationsFor: (id) async => [_note(id: 'n1', quote: 'match too')],
        );

        expect(
          hits.where((hit) => hit.document.metadata.id == 'a'),
          hasLength(1),
        );
        expect(hits.first.kind, LibrarySearchHitKind.metadata);
      },
    ...
    test(
      'empty query short-circuits metadata hits and skips annotation scan',
      () async {
        final docs = [
          _doc(id: 'a', title: 'Alpha'),
          _doc(id: 'b', title: 'Beta'),
    ...
    test('documents whose metadata AND notes both match are returned once as metadata', () async {
      final docs = [_doc(id: 'a', title: 'match')];
    ...
    test(
      'a document is only added once when both author and title match',
      () async {
        final docs = [_doc(id: 'book', title: 'match', author: 'match')];
    ...
    test(
      'multiple matching notes in one document collapse to one hit',
      () async {
        final docs = [_doc(id: 'a', title: 'Unrelated')];
    ...
    test(
      'empty query in the sync API returns all docs sorted by lastOpened',
      () {
        final docs = [
        ...
        expect(
          hits.map((hit) => hit.document.metadata.id).toList(),
          equals(['new', 'old']),
        );
        expect(
          hits.every((hit) => hit.kind == LibrarySearchHitKind.metadata),
    ...
    test('sync API with non-empty query and a non-null annotationsFor returns empty', () {
      ...
      final docs = [_doc(id: 'book1', title: 'Moby Dick', author: 'Melville')];
