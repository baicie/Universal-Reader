...
void main() {
    ...
    test('throws StateError when the host has not overridden it', () {
      ...
      try {
        ...
        expect(
          e.toString(),
          contains(
            'Bad state: Library repository must be overridden at startup',
          ),
        );
      }
    });
    ...
    test('throws StateError when the host has not overridden it', () {
      ...
      try {
        ...
        expect(
          e.toString(),
          contains(
            'Bad state: AI settings repository must be overridden at startup',
          ),
        );
      }
    });
  ...
  group('libraryProvider', () {
    test(
      'wires the overridden repository into a PersistedLibraryController',
      () async {
        ...
        addTearDown(container.dispose);
        final controller = container.read(libraryProvider);
    ...
    test(
      'honours the overridden shelf repository when toggling favorites',
      () async {
        final repository = InMemoryLibraryRepository();
    ...
    test('watches the annotation repository provider', () async {
      ...
      await controller.waitUntilReady();
      expect(
        controller.annotationRepository,
        isA<InMemoryAnnotationRepository>(),
      );
    });
  });
  ...
  group('aiSettingsProvider', () {
    test(
      'wires the overridden repository into an AiSettingsController',
      () async {
        final container = ProviderContainer(
          overrides: [
  ...
  group('aiRuntimeProvider', () {
    test(
      'defaults to a local AiRuntime backed by an in-memory conversation store',
      () {
        final container = ProviderContainer();
        addTearDown(container.dispose);
