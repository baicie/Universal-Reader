...
void main() {
  test(
    'usesRemoteStore is false even though it persists to SharedPreferences',
    () async {
  ...
  test(
    'importBytes writes a base64 cover key when cover bytes are provided',
    () async {
      SharedPreferences.setMockInitialValues({});
      final prefs = await SharedPreferences.getInstance();
      final repo = WebPersistentLibraryRepository(prefs);
  ...
  test(
    'importBytes with duplicate content returns the existing record',
    () async {
      SharedPreferences.setMockInitialValues({});
      final repo = WebPersistentLibraryRepository(
        await SharedPreferences.getInstance(),
  ...
  test(
    'writeReadingState updates the catalog without touching the bytes',
    () async {
      SharedPreferences.setMockInitialValues({});
      final prefs = await SharedPreferences.getInstance();
      final repo = WebPersistentLibraryRepository(prefs);
  ...
  test(
    'writeIdentity updates the catalog and leaves bytes untouched',
    () async {
      SharedPreferences.setMockInitialValues({});
      final prefs = await SharedPreferences.getInstance();
      final repo = WebPersistentLibraryRepository(prefs);
  ...
  test(
    'delete clears the catalog entry plus the file and cover keys',
    () async {
      SharedPreferences.setMockInitialValues({});
      final prefs = await SharedPreferences.getInstance();
      final repo = WebPersistentLibraryRepository(prefs);
